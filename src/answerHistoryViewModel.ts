        ...
        const earlierAnswersAreUnmarked = (submittedAtMillis: number) => {
            return allAnswersForQuestion.some((item) => {
                return item.data.correct === undefined && // Item is umarked...
                    item.data.submittedAt && item.data.submittedAt.toMillis() < submittedAtMillis && // ...and submitted earlier...
                    cluesById[item.data.clueId] && answerIsValid(item.data, cluesById[item.data.clueId].data) && // ...and is valid...
