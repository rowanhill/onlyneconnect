...
describe('/quiz/{quiz}/clues security ruleset', () => {
    ...
    describe('for a quiz owned by the user', () => {
        ...
        it('allows create', async () => {
            await assertSucceeds(testDb.collection(`/quizzes/${quizId}/clues`).add({ dummy: 'clue' }));
        });

        it('allows delete', async () => {
            const doc = await adminDb.collection(`/quizzes/${quizId}/clues`).add({ dummy: 'clue' });
            await assertSucceeds(testDb.doc(doc.path).delete());
        });

        it('allows update', async () => {
            const doc = await adminDb.collection(`/quizzes/${quizId}/clues`).add({ dummy: 'clue' });
            await assertSucceeds(testDb.doc(doc.path).update({ dummy: 'updated clue' }));
        });

        it('allows fetching even unrevealed clues', async () => {
            const doc = await adminDb.collection(`/quizzes/${quizId}/clues`).add({ isRevealed: false });
    ...
    describe('for a quiz owned by someone else', () => {
        ...
        it('denies create', async () => {
            await assertFails(testDb.collection(`/quizzes/${quizId}/clues`).add({ dummy: 'clue' }));
        });
        ...
        describe('when the user is a player in the quiz', () => {
            ...
            it('allows fetching revealed clues', async () => {
                const doc = await adminDb.collection(`/quizzes/${quizId}/clues`).add({ isRevealed: true });
                await assertSucceeds(testDb.doc(doc.path).get());
            });

            it('denies fetching unrevealed clues', async () => {
                const doc = await adminDb.collection(`/quizzes/${quizId}/clues`).add({ isRevealed: false });
                await assertFails(testDb.doc(doc.path).get());
            });

            it('allows listing revealed clues', async () => {
                const doc = await adminDb.collection(`/quizzes/${quizId}/clues`).add({ isRevealed: true });
                await adminDb.collection(`/quizzes/${quizId}/clues`).add({ isRevealed: false });
                await assertSucceeds(testDb.collection(doc.parent.path).where('isRevealed', '==', true).get());
            });

            it('denies listing unrevealed clues', async () => {
                const doc = await adminDb.collection(`/quizzes/${quizId}/clues`).add({ isRevealed: true });
                await adminDb.collection(`/quizzes/${quizId}/clues`).add({ isRevealed: false });
